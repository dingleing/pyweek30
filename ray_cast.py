...
class Rays:
    ...
    def cast_rays(self, number_of_rays, starting_ray_angle, Player_mid, game_map, player_angle, ray_dict):
        ...
        for i in range(number_of_rays):
            ...
            if self.player_angle > self.pi:
                ...
                self.rays[i].ray_cords[0] = round(((Player_mid[1] - self.rays[i].ray_cords[1]) * aTan) + Player_mid[0],
                                                  3)
            ...
            while going < self.ray_render:
                ...
                try:
                    ...
                    if game_map[hit_pos[1]][hit_pos[0]] in self.solid:
                        ...
                        transparent_hits[0].append([copy.copy(self.rays[i].ray_cords),
                                                    distance_indicator_precise(Player_mid,
            ...
            if self.player_angle > self.three_halves_pi or self.player_angle < self.half_pi:
                ...
                self.rays[i].ray_cords[1] = round(-((Player_mid[0] - self.rays[i].ray_cords[0]) * tan) + Player_mid[1],
                                                  3)
            ...
            while to_go < self.ray_render:
                ...
                try:
                    ...
                    if game_map[hit_pos0[1]][hit_pos0[0]] in self.solid:
                        ...
                        transparent_hits[1].append([copy.copy(self.rays[i].ray_cords),
                                                    distance_indicator_precise(Player_mid,
                                                                               copy.copy(self.rays[i].ray_cords)),
            ...
            if first_ray_dist < self.limit or second_ray_dist < self.limit:
                if first_ray_dist > second_ray_dist:
                    ...
                    self.draw_rays_transparent(transparent_hits, ray_dict, [self.rays[i].angle, player_angle], i,
                                               second_ray_dist)
    ...
    def draw_rays_transparent(self, transparent_hits, ray_dict, angles, i, final_ray_dist):
        ...
        for num_of_item in range(len(transparent_hits)):
            transparent_hits[num_of_item][1] = self.better_distance(transparent_hits[num_of_item][1],
                                                                    angles[0], angles[1])
        ...
        for coll in transparent_hits:
            if coll[1] < final_ray_dist:
                ...
                if ray_dict[coll[2]].image:
                    ...
                    self.display.blit(pygame.transform.scale(ray_dict[coll[2]].slice_textures[slice_of_image],
                                                             [self.ray_width, height]),
                                      ((self.ray_width * i), 225 - (height / 2)))
                else:
    ...
    def sort_transparent_hits(t_hits):
        ...
        for i in range(1, len(all_together)):
            while all_together[i][1] > all_together[i - 1][1] and i > 0:
                all_together[i], all_together[i - 1] = all_together[i - 1], all_together[i]
                i -= 1
