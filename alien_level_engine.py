...
class Objects:
    ...
    # this func is utter garbage like collisions_in_place dont use in real life
    def do_inplace_col(self, objects):
        for obj in self.collision_objects:
            collision_in_place(obj, objects)
...
class Collisions(Id):
    ...
    def hit_bottom(self, obj, objects):
        if self.type == "solid":
            ...
            if obj.type == "player":
                obj.move.speed += 1
                obj.move.angle_speed = obj.move.speed/160
                hit_pos = [int(self.object_pos[1] // 32), int(self.object_pos[0] // 32)]
                objects.values["pos_to_del"].append(hit_pos)
                objects.objects_to_delete.append(self.object_id)
...
class Moving_Object:
    def __init__(self):
        ...
        self.speed = 1
        self.angle_speed = self.speed/160
        self.offset = 0
...
class Timers:
    ...
    def add_time(self, ray_dict, objects):
    ...
    for line in game_map:
        for obj in line:
            ...
            if obj in ["1", "2", "3", "4", "a", "b", "c"]:
                ...
                obj = Object("speed_up", game.custom_id_giver, [x, y], [0, 0], 0, False, [width, height])
                sort(obj, objects)
                game.custom_id_giver += 1
            elif obj == "7":
                obj = Object("teleporter", game.custom_id_giver, [x, y], [0, 0], 0, False, [width, height])
                sort(obj, objects)
                game.custom_id_giver += 1
            elif obj == "9":
                obj = Object("solid", game.custom_id_giver, [x, y], [0, 0], 0, False, [width, height])
                obj.memory.append("door")
                sort(obj, objects)
                game.custom_id_giver += 1
            x += width
        y += height
...
def get_ray_dictionary():
    ...
    blocks = {
        "0": Ray_cast_block(),
        "1": Ray_cast_block(pygame.image.load("assets/textures/alien_level/alien_wall.png")),
        ...
        "6": Ray_cast_block(pygame.image.load("assets/textures/alien_level/alien_key.png")),
        "7": Ray_cast_block(pygame.image.load("assets/textures/alien_level/abanonded_ship.png")),
        "8": Ray_cast_block(pygame.image.load("assets/textures/speed_up_temporary.png")),
        # wall1 or "9" is used for doors
        "9": Ray_cast_block(pygame.image.load("assets/textures/alien_level/alien_wall1.png")),
        "a": Ray_cast_block(pygame.image.load("assets/textures/alien_level/alien_wall0.png")),
        "b": Ray_cast_block(pygame.image.load("assets/textures/alien_level/alien_wall2.png")),
        "c": Ray_cast_block(pygame.image.load("assets/textures/alien_level/alien_wall3.png"))
        }
...
# this function is complete garbage only made cause deadline is today never use this in real life
def collision_in_place(obj, objects):
    # collisions for left/right
    hit_list = find_collisions(obj, objects)
    for item in hit_list:
        item.hit_right(obj, objects)
